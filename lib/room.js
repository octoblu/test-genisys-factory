...
class Room {
  ...
  getRoomState(callback) {
    this.meshblu.device(this.room.roomId, function (error, device){
      ...
      var roomState = {
        currentMeeting : _.get(device, 'genisys.currentMeeting'),
        meetingUrl     : _.get(device, 'genisys.currentMeeting.meetingUrl'),
        inSkype        : _.get(device, 'genisys.inSkype'),
        subject        : _.get(device, 'genisys.currentMeeting.subject'),
      }
      return callback(roomState)
    })
  }
