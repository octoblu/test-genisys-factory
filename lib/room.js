...
class Room {
  ...
  resetRoom(timeout, callback) {
    ...
    this.getRoomState((error, roomState) => {
      ...
      if (_.isEmpty(currentMeeting)) return callback(null, true)

      this.triggerButtonPress(timeout,(error) => {
  ...
  triggerButtonPress(timeout,callback) {
