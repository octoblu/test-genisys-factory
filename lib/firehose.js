...
var room = new Room({ room: roomOptions, meshbluConfig: meshbluConfig, inquisitorFile : inquisitorFile });
// var messageHandler = sinon.stub()
var message = {}
const buttonTimeout = 25000;
...
describe('Ad hoc meeting test:', function() {
  ...
  before('Get current meeting', function(done) {
    room.listenCurrentMeeting(function(roomState) {
      currentRoomState = roomState
      console.log('got current meeting')
      done()
    })
  })

  it('should have a current meeting object', function(done) {
    expect(currentRoomState.genisys.currentMeeting).to.exist
    done()
  })

  it('should have subject of \'Instant Meeting\'', function(done) {
    expect(currentRoomState.genisys.currentMeeting.subject).to.equal('Instant Meeting')
    done()
  })
