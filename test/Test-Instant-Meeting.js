...
var inquisitorFile = require('../config/Cybernet/inquisitor.json')
var btnCredFile = require('../config/Cybernet/button.json')
var roomOptions = require('../config/Cybernet/room.json')
var meshbluConfig = require('../config/meshblu-citrix.json')
var Room = require('../lib/room.js')
var _ = require('lodash')

var room = new Room({room: roomOptions, meshbluConfig: meshbluConfig, inquisitorFile: inquisitorFile, btnCredFile: btnCredFile});
var message = {}
...
describe('Instant meeting with Skype:' , function() {
    ...
    before('Start an Instant meeting:', function(done) {
        room.triggerButtonPress(function(error) {
            ...
            room.waitForMeeting('Booked', function(error, result) {
                if (error)
                    done(error)
    ...
    before('Get the light status', function(done) {
        room.waitForLight('Red', function(error, color) {
            actualColor = color
            if (error)
                done(error)
            done()
        ...
        describe('Verify that Skype is Ending successfully', function() {
            before('End Skype Session', function(done) {
                room.endSkype(function(error) {
                    if (error)
                        console.log('Error ending skype : ', error);
    ...
    describe('Verify that Meeting is Ending successfully', function() {
        ...
        before('End Meeting', function(done) {
            room.triggerButtonPress(function(error) {
                ...
                room.waitForMeeting('Available', function(error, result) {
                    if (error)
                        done(error)
                    room.getRoomState(function(error, state) {
                        if (error)
